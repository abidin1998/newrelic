...
public class CalculateMostRepeatedWordsUnicode {
    ...
    public static void main(String[] args) {
        ...
        // Read input from stdin if there is no file
    ...
    private static StringBuilder getProcessedText(List<String> text) {
        ...
        for (String line : text) {
            // "\\p{Punct}" this regex expression is supposed to delete all punctuation symbols
            // but doesn't give the same numbers as the default version so this is just a try
            sb.append(line.replaceAll("\\p{Punct}", "").toLowerCase());
            sb.append(" ");
        }
        return sb;
    }
